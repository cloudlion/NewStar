...
namespace GameUtil
{
    static public class MathUtil
    {
        ...
        {
            return StrToVec2(str, '\\');
        }
        public static Vector2 StrToVec2(string str, char separator)
        {
            SysUtil.Assert(str != null, "null str to vector2");

            string[] vals = str.Split(separator);
            SysUtil.Assert(vals.Length == 2, "bad format str \"" + str + "\" to vector2");
        ...
        public static bool RectIntersect(Rect rt0, Rect rt1)
        {
            return rt0.Contains(new Vector2(rt1.x, rt1.y)) ||
                   rt0.Contains(new Vector2(rt1.x, rt1.y + rt1.height)) ||
                   rt0.Contains(new Vector2(rt1.x + rt1.width, rt1.y)) ||
                   rt0.Contains(new Vector2(rt1.x + rt1.width, rt1.y + rt1.height)) ||
                   rt1.Contains(new Vector2(rt0.x, rt0.y)) ||
                   rt1.Contains(new Vector2(rt0.x, rt0.y + rt0.height)) ||
                   rt1.Contains(new Vector2(rt0.x + rt0.width, rt0.y)) ||
                   rt1.Contains(new Vector2(rt0.x + rt0.width, rt0.y + rt0.height));
        }
        ...
        {
            if (bound0.min.x > bound1.max.x)
            {
                return false;
            }
            if (bound0.min.y > bound1.max.y)
            {
                return false;
            }
            if (bound0.min.z > bound1.max.z)
            {
                return false;
            }

            if (bound0.max.x < bound1.min.x)
            {
                return false;
            }
            if (bound0.max.y < bound1.min.y)
            {
                return false;
            }
            if (bound0.max.z < bound1.min.z)
            {
                return false;
            }
        ...
        public static bool BoundIntersect(Bounds bound0, Bounds bound1, ref Bounds intersectBounds)
        {
            ...
            if (intersectMax.x <= intersectMin.x)
            {
                return false;
            }
            if (intersectMax.y <= intersectMin.y)
            {
                return false;
            }
            if (intersectMax.z <= intersectMin.z)
            {
                return false;
            }
        ...
        {
            return FloatEqual(bd.size.x, 0.0f) || FloatEqual(bd.size.x, 0.0f) || FloatEqual(bd.size.x, 0.0f);
        }
        ...
        public static void CalMaskTex(Texture2D tex, Color colMask, Color colNoMask, float angle)
        {
            ...
            {
                Logger.LogWarning("invalid tex format: " + tex.format);
                return;
            }
        ...
        public static int HexChar2Int(char c)
        {
            ...
            Logger.LogError("invalid char format: " + c);
            return 0;
        }
                                                ...
                                                out Vector2 cross)
        {
            ...
            {
                Logger.LogWarning("segment in rect: " + src + "" + dst + ""
                                  + min + "" + max);
                return false;
            }
